...
For Gates-style packages, load reviewed FPocket/P2Rank geometry with
`import_gates_pocket_outputs` after the curated scores and structures are in
place. The reusable Nodo0 workflow and resume commands live in
[`docs/CURATED_GENOME_IMPORT.md`](CURATED_GENOME_IMPORT.md).

Do not import SLURM recalculation results when the reviewed package already
contains equivalent outputs.
