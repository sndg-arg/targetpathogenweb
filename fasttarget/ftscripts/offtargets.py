...
def microbiome_offtarget_blast_species (databases_path, output_path, organism_name, cpus=multiprocessing.cpu_count()):
    ...
    for genome_dir in sorted(os.listdir(species_databases_path)):
        ...
        # One flaky species DB (or a transient node-level crash -- diamond
        # has been observed to SIGSEGV on a perfectly valid database/query
        # pair, apparently node-specific rather than data corruption) must
        # not abort the whole species catalogue scan: with ~4700 species to
        # get through, losing everything already computed to one bad
        # iteration is far worse than skipping that one species and moving
        # on. The "skip if exists" check above already makes a full retry of
        # the whole run resume past completed species; this is the
        # per-species equivalent of that same resilience.
        try:
            ...
            print(f"✗ Diamond BLAST failed against {genome_dir}, skipping: {e}")
            logging.exception(f"Diamond BLAST failed against {genome_dir}: {e}")
            if os.path.exists(blast_output_path):
