...
for genome_path in $DATA_DIR/*/*; do
    ...
    --load copy --out /jbrowse/data --type bgzipFasta

    # Comando para agregar el track de anotaciones (GFF), especificando el nombre de la asamblea
    docker run -v $DATA_DIR:/data --rm -u $(id -u):$(id -g) -v $JBROWSE_DIR:/jbrowse jbrowse \
    jbrowse add-track /data/${genome_path#$DATA_DIR/}/${genome_id}.gff.bgz \
    --load copy --out /jbrowse/data --assemblyNames ${genome_id}
done
