...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for protein in tqdm(proteins, total=len(proteins)):
            ...
            structures = list(
                BioentryStructure.objects.filter(bioentry_id=bioentry_id).select_related("pdb")
            )
            if not structures:
                continue
            ...
            experimental_links = [
                ...
                if classify_structure_experiment(structure.pdb.experiment)
                == STRUCTURE_SOURCE_EXPERIMENTAL
            ]
            model_links = [
                structure for structure in structures if structure not in experimental_links
            ]
