...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Biodatabase.objects.get(name=accession)
        ...
        self.index_ontology = Ontology.objects.get_or_create(
            name=Ontology.BIOINDEX, definition="Pre calculated values"
        )[0]

        with tqdm(
            Bioentry.objects.filter(biodatabase__name=bioprotdb, structures__isnull=False)
        ) as pbar:
            for _i, p in enumerate(pbar):
                pbar.set_description(p.name)
                structure = (
                    p.structures.prefetch_related(
                        "pdb__residue_sets__residue_set_residue", "pdb__residue_sets__residue_set"
                    )
                    .all()[0]
                    .pdb
                )
    ...
    def process_prot(self, bioentry, structure):
        ...
        ScoreParam.objects.filter(category="Structure", name="druggability").get()

        for _rs in [
            x for x in structure.residue_sets.all() if x.residue_set.name == "FPocketPocket"
        ]:
            pass
        """
