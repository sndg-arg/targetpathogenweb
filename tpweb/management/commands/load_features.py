...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "features_tsv", help="tsv with: feature_type feature_id resids prop1 prop2 ... "
        )
    ...
    def handle(self, *args, **options):
        ...
        assert os.path.exists(options["residueset_tsv"]), (
            f'"{options["residueset_tsv"]}" does not exists!'
        )
        ...
        for _, r in df.iterrows():
            with transaction.atomic():
                ...
                prs.save()
                residues_dict = {
                    res.chain + "_" + str(res.resid): res for res in pdb.residues.all()
                }

                residues = [
                    residues_dict[x] for x in residues_dict if x in r.chain_resids.split(",")
                ]
                for res in residues:
