    ...
    def handle(self, *args, **options):
        ...
        for _, record in tqdm(list(df.iterrows())):
            ...
            if not os.path.exists(fpo._info_file_path()):
                cmd = fpocket_template.format(
                    pdbinput=absolute_pdb_input, workdir=absolute_pocket_path
                )
            ...
            fixed_files = [
                os.path.join(fpocket_folder, f"pocket{i}_info_fixed.txt")
                for i in range(1, len(_out_folders) + 1)
            ]
